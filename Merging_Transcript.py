...
def dead_miRNA(miRNA):
  ...
  '''
  if "Mir" in miRNA:
    if "mmu-mir-"+miRNA.split("Mir")[1] in mdead:
      return 1
  elif "mmu-mi" in miRNA:
    try:
      if "mmu-mir-"+miRNA.split("miR")[1].strip("-") in mdead:
        print >>sys.stderr,"mmu-mir-"+miRNA.split("miR")[1].strip("-")
        return 1
    except:
      if miRNA in mdead:
        return 1
...
def Main():
  ...
  while True:
    ...
    if line=="":break
      ...
      mdead.append(line.strip().split("   ")[1])
